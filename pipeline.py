...
def clean_data(df):
    ...
    # Unificar columnas de causa y eliminar las originales
    causa_cols_to_drop = []
    if "Descripcio_causa_mediata" in df.columns and "Causa conductor" in df.columns:
        df['Causa'] = df['Descripcio_causa_mediata'].fillna(df['Causa conductor'])
        causa_cols_to_drop.extend(["Descripcio_causa_mediata", "Causa conductor"])
    elif "Descripcio_causa_mediata" in df.columns:
        df['Causa'] = df['Descripcio_causa_mediata']
        causa_cols_to_drop.append("Descripcio_causa_mediata")
    elif "Causa conductor" in df.columns:
        df['Causa'] = df['Causa conductor']
        causa_cols_to_drop.append("Causa conductor")
    
    if causa_cols_to_drop:
        df = df.drop(columns=causa_cols_to_drop, errors='ignore')
    else:
